...
class EventHandlerString : public EventHandlerImpl, public Singleton<EventHandlerString>
{
	...
	public:
		...
		*/
		void getFrameEvent(std::vector<std::string>& buffer);
		...
		*/
		void addFrameEvent(std::string literalEvent);
		...
		*/
		void addEvent(std::string eventName, Event::InputType call, int key = -1, uint8_t config = Event::DOWN_FLAG | Event::BUTTON);
	...
	private:
		...
		*/
		EventHandlerString(const std::string& path = "Resources/" );
		...
		*/
		...
		//  Attributes
		std::map<Event*, std::string> eventMapping;		//!< Container for assosiate Event object -> user event
		std::multimap<std::string, Event*> userMapping;	//!< Container for assosiate user event -> Event object
		std::vector<std::string> frameEvent;			//!< The list of all event occured and non treated yet
		std::vector<std::string> frameEventBuffer;		//!< A buffer used for thread safe uses
		//
};
