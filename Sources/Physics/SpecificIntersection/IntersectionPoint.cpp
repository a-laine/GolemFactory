...
Intersection::Contact Intersection::intersect_PointvsTriangle(const glm::vec3& point, const glm::vec3& triangle1, const glm::vec3& triangle2, const glm::vec3& triangle3, glm::vec3* e1, glm::vec3* e2)
{
	// compute variables and get barrycentric coordinates of projected point
	...
	if (glm::length2(n) <= COLLISION_EPSILON) // flat triangle
	{
		...
		if (d1 >= d2 && d1 >= d3)
		{
			Contact contact = intersect_PointvsSegment(point, triangle1, triangle2);
			if (contact.contactPointB == triangle1) // maybe will cause error : change into       glm::length2(contact.contactPointB - triangle1) < COLLISION_EPSILON
			{
				e1 = (glm::vec3*)&triangle1;
				e2 = nullptr;
			}
			else if (contact.contactPointB == triangle2)
			{
				e1 = (glm::vec3*)&triangle2;
				e2 = nullptr;
			}
			else
			{
				e1 = (glm::vec3*)&triangle1;
				e2 = (glm::vec3*)&triangle2;
			}
			return contact;
		}
		else if (d2 >= d1 && d2 >= d3)
		{
			Contact contact = intersect_PointvsSegment(point, triangle1, triangle3);
			if (contact.contactPointB == triangle1)
			{
				e1 = (glm::vec3*)&triangle1;
				e2 = nullptr;
			}
			else if (contact.contactPointB == triangle3)
			{
				e1 = (glm::vec3*)&triangle3;
				e2 = nullptr;
			}
			else
			{
				e1 = (glm::vec3*)&triangle1;
				e2 = (glm::vec3*)&triangle3;
			}
			return contact;
		}
		else
		{
			Contact contact = intersect_PointvsSegment(point, triangle3, triangle2);
			if (contact.contactPointB == triangle3)
			{
				e1 = (glm::vec3*)&triangle3;
				e2 = nullptr;
			}
	...
	if (barry.x < 0.f || barry.y < 0.f || barry.x + barry.y > 1.f) // projection out of triangle
	{
		...
		// if necessarry compute intermediate result : closest is a corner
		if (glm::length2(p1 - triangle1) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle1;
			e2 = nullptr;
		}
		else if (glm::length2(p1 - triangle2) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle2;
			e2 = nullptr;
		}
		if (glm::length2(p2 - triangle3) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle3;
			e2 = nullptr;
		}
		else if (glm::length2(p2 - triangle2) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle2;
			e2 = nullptr;
		}
		if (glm::length2(p3 - triangle3) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle3;
			e2 = nullptr;
		}
		else if (glm::length2(p3 - triangle1) < COLLISION_EPSILON)
		{
			e1 = (glm::vec3*)&triangle1;
			e2 = nullptr;
		}
		...
		// choose the best candidate and compute intermediate result : closest is an edge
		if (d1 <= d2 && d1 <= d3)
		{
			p = p1;
			if (!e1)
