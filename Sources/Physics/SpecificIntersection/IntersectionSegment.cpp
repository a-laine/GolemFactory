...
Intersection::Contact Intersection::intersect_SegmentvsTriangle(const glm::vec3& segment1, const glm::vec3& segment2, const glm::vec3& triangle1, const glm::vec3& triangle2, const glm::vec3& triangle3)
{
	...
	if (n == glm::vec3(0.f)) 
	{
		...
		if (d1 >= d2 && d1 >= d3)
			return intersect_SegmentvsSegment(segment1, segment2, triangle1, triangle2);
		else if (d2 >= d1 && d2 >= d3)
			return intersect_SegmentvsSegment(segment1, segment2, triangle1, triangle3);
		else
			return intersect_SegmentvsSegment(segment1, segment2, triangle3, triangle2);
	}

	// eliminate collision case
	//glm::vec3 intersection;
	CollisionReport dummyReport;
	if (Collision::collide_SegmentvsTriangle(segment1, segment2, triangle1, triangle2, triangle3, &dummyReport))
	{
		Contact contact;
		contact.contactPointA = dummyReport.points.back() + triangle1;
	...
	/*std::pair<glm::vec3, glm::vec3> s1 = CollisionUtils::getSegmentsClosestSegment(segment1, segment2, triangle1, triangle2);
	std::pair<glm::vec3, glm::vec3> s2 = CollisionUtils::getSegmentsClosestSegment(segment1, segment2, triangle1, triangle3);
	std::pair<glm::vec3, glm::vec3> s3 = CollisionUtils::getSegmentsClosestSegment(segment1, segment2, triangle2, triangle3);
...
Intersection::Contact Intersection::intersect_SegmentvsAxisAlignedBox(const glm::vec3& segment1, const glm::vec3& segment2, const glm::vec3& boxMin, const glm::vec3& boxMax)
{
	...
	/*if (Collision::collide_SegmentvsAxisAlignedBox(segment1, segment2, boxMin, boxMax))
	{
		...
		Contact contact;
		contact.contactPointA = CollisionUtils::getSegmentClosestPoint(segment1, segment2, bcenter);
...
Intersection::Contact Intersection::intersect_SegmentvsSphere(const glm::vec3& segment1, const glm::vec3& segment2, const glm::vec3& sphereCenter, const float& sphereRadius)
{
	...
	Contact contact;
	contact.contactPointA = CollisionUtils::getSegmentClosestPoint(segment1, segment2, sphereCenter);
