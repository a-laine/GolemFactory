			...
			public:
				...
				struct Face
				{
					Face(const glm::vec3& A1 = glm::vec3(0.f), const glm::vec3& A2 = glm::vec3(0.f), const glm::vec3& A3 = glm::vec3(0.f),
						 ...
						 a1(A1), a2(A2), a3(A3), b1(B1), b2(B2), b3(B3), n(normal), onHull(true), e1(nullptr), e2(nullptr), e3(nullptr) {};
				...
				//	Public functions
				void init(std::vector<std::pair<glm::vec3, glm::vec3>> simplex);
				bool add(const glm::vec3& a, const glm::vec3& b);
				Face* getClosestFromOrigin();
				void draw(const glm::vec3& offset);

				bool isNewPair(const glm::vec3& a, const glm::vec3& b);
				bool isNewPoint(const glm::vec3& p);
