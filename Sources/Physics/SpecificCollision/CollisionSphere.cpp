...
bool Collision::collide_SpherevsOrientedBox(const vec4f& sphereCenter, const float& sphereRadius, const mat4f& boxTranform, const vec4f& boxMin, const vec4f& boxMax, CollisionReport* report)
{
	...
	quatf rot = boxTranform.extractRotation();
	vec4f p = conjugate(rot) * (sphereCenter - center);
	vec4f clamped = vec4f::clamp(p, -halfSize, halfSize);
	vec4f closest = center + rot * clamped;
	...
	else
	{
		if (report)
		{
			...
			{
				delta = halfSize - vec4f::abs(clamped);
				if (delta.x < delta.y && delta.x < delta.z)
				{
					report->depths.push_back(sphereRadius + delta.x);
					report->normal = clamped.x > 0.f ? boxTranform[0] : -boxTranform[0];
				}
				else if (delta.y < delta.x&& delta.y < delta.z)
				{
					report->depths.push_back(sphereRadius + delta.y);
					report->normal = clamped.y > 0.f ? boxTranform[1] : -boxTranform[1];
				}
				else
				{
					report->depths.push_back(sphereRadius + delta.z);
					report->normal = clamped.z > 0.f ? boxTranform[2] : -boxTranform[2];
				}
				report->points.push_back(sphereCenter - sphereRadius * report->normal);
			}
			else
