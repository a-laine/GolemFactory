...
void drawCylinder(vec3 begin, vec3 end)
{
	...
	vec3 previous = center + radius[0] * normal;
	for(int i = 1; i < QUADRATURE + 1; i++)
	{
		tmp = center + radius[0] * cos(2 * M_PI * i / QUADRATURE) * normal + radius[0] * sin(2 * M_PI * i / QUADRATURE) * tangent;
...
void drawHemisphere(vec3 center, vec3 direction, vec3 normal)
{
	...
	{
		v4 = center + 
			radius[0] * cos(M_PI / 2 * j / QUADRATURE_S) * normal +
			radius[0] * sin(M_PI / 2 * j / QUADRATURE_S) * direction;
			
		v3 = center + 
			radius[0] * cos(M_PI / 2 * (j + 1) / QUADRATURE_S) * normal +
			radius[0] * sin(M_PI / 2 * (j + 1) / QUADRATURE_S) * direction;
				
		for(int i = 1; i < QUADRATURE+1; i++)
		{
			// compute new vectors
			v1 = center + 
				radius[0] * cos(2 * M_PI * i / QUADRATURE) * cos(M_PI / 2 * j / QUADRATURE_S) * normal + 
				radius[0] * sin(2 * M_PI * i / QUADRATURE) * cos(M_PI / 2 * j / QUADRATURE_S) * tangent + 
				radius[0] * sin(M_PI / 2 * j / QUADRATURE_S) * direction;
				
			v2 = center + 
				radius[0] * cos(2 * M_PI * i / QUADRATURE) * cos(M_PI / 2 * (j + 1) / QUADRATURE_S) * normal + 
				radius[0] * sin(2 * M_PI * i / QUADRATURE) * cos(M_PI / 2 * (j + 1) / QUADRATURE_S) * tangent + 
				radius[0] * sin(M_PI / 2 * (j + 1) / QUADRATURE_S) * direction;
