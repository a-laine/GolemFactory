WiredInstanced
{
	...
	vertex :
	{
		...
		uniform mat4 model[MAX_INSTANCE];
		...
		void main()
		{
			...
			gl_Position = projection * view * M * position;
			fragmentNormal_gs = normalize(transpose(inverse(M)) * normal);
			fragmentPosition_gs = M * position;
		}
	};
	geometry : 
	{
